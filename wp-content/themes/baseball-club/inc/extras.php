 ...
 * @return array
 */
function baseballclub_body_classes( $classes ) {
...
add_filter( 'body_class', 'baseballclub_body_classes' );
...
if (!function_exists('baseballclub_str_replace_assoc')) {

    /**
     * baseballclub_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function baseballclub_str_replace_assoc(array $replace, $subject) {
 ...
 */
function baseballclub_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html__('0','baseball-club'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'baseballclub_add_to_cart_fragment' );
...
if ( ! function_exists( 'baseballclub_breadcrumb_title' ) ) {
	function baseballclub_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'baseball-club' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'baseball-club' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'baseball-club' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'baseball-club' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'baseball-club' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'baseball-club' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'baseball-club' ), (get_the_author( '', false ) ));		
...
function baseballclub_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','baseball-club'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','baseball-club'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','baseball-club'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','baseball-club').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','baseball-club').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','baseball-club').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','baseball-club').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','baseball-club'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','baseball-club') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_baseballclub_dismissed_notice_handler', 'baseballclub_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function baseballclub_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'baseballclub_admin_install_plugin' );

function baseballclub_admin_install_plugin() {
    /**
     * Install Plugin.
     */
